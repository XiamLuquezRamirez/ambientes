...
.est-nombre  { font-weight: 600; color: #1E293B; font-size: .9rem; }
.est-sub     { font-size: .76rem; color: #64748B; }
.est-estado-badge {
    font-size: .72rem; padding: 2px 10px; border-radius: 99px; font-weight: 600;
}
.badge-activo      { background: #ECFDF5; color: #065F46; }
.badge-restringido { background: #FEF3C7; color: #92400E; }
.badge-adaptado    { background: #EFF6FF; color: #1E40AF; }
...
.busq-fila:hover { background: #F8FAFC; }
.busq-fila.marcado { background: #EFF6FF; }
.busq-check { width:16px; height:16px; accent-color:#2563EB; flex-shrink:0; }
</style>
@endpush
...
const URL_STORE    = "{{ route('admin.ambientes.asignaciones.store', $ambiente) }}";
const URL_UPDATE   = "{{ route('admin.ambientes.asignaciones.update', [$ambiente, ':ea']) }}";
const URL_DESTROY  = "{{ route('admin.ambientes.asignaciones.destroy', [$ambiente, ':ea']) }}";
...
async function cargarLista() {
    ...
    const { data } = await apiFetch(`${URL_TABLA}?anio=${ANIO_LECTIVO}`);
    if (!data.ok) {
    ...
    const badgeClase = { activo:'badge-activo', restringido:'badge-restringido', adaptado:'badge-adaptado' };
    const badgeTxt   = { activo:'Activo', restringido:'Restringido', adaptado:'Adaptado' };
        ...
        <div class="est-row" id="ea-row-${a.id}">
            ...
            <div style="display:flex;gap:6px;flex-shrink:0;margin-left:8px">
                <button class="btn-accion btn-editar"
                        onclick="abrirEditarEa(${a.id}, '${a.nombre.replace("'","\\'")}', '${a.estado}', ${JSON.stringify(a.observacion ?? '')})"
                        title="Editar estado">
                    <i class="fa-solid fa-pencil"></i>
                </button>
                <button class="btn-accion btn-eliminar"
                        onclick="quitarEstudiante(${a.id}, '${a.nombre.replace("'","\\'")}')">
...
async function buscarParaAgregar(termino) {
    clearTimeout(_debBusq);
    _debBusq = setTimeout(async () => {
        const { data } = await apiFetch(`${URL_BUSCAR}?anio=${ANIO_LECTIVO}&q=${encodeURIComponent(termino)}`);
        if (!data.ok) return;
...
function renderBuscarLista() {
    ...
    cont.innerHTML = _todosAgregar.map(e => `
        <label class="busq-fila ${_selAgregar.has(e.id) ? 'marcado' : ''}"
               data-id="${e.id}"
               onclick="toggleSelAgregar(${e.id}, this)">
            <input type="checkbox" class="busq-check" ${_selAgregar.has(e.id) ? 'checked' : ''}
                   onclick="event.stopPropagation();toggleSelAgregar(${e.id},this.closest('.busq-fila'))">
            ...
            <div>
                <div style="font-weight:600;font-size:.88rem;color:#1E293B">${e.nombre}</div>
...
function toggleSelAgregar(id, fila) {
...
async function confirmarAgregar() {
    ...
    const { data } = await apiFetch(URL_STORE, 'POST', {
...
let _modalEstadoEaBS = null, _eaEditId = null;

function abrirEditarEa(id, nombre, estado, obs) {
    _eaEditId = id;
    document.getElementById('ctxEstadoEa').textContent = nombre;
    document.getElementById('selEstadoEa').value       = estado;
    document.getElementById('txtObsEa').value          = obs ?? '';
...
/* ── Quitar estudiante ───────────────────────────────── */
async function quitarEstudiante(id, nombre) {
