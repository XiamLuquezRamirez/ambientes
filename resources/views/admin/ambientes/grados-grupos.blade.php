    ...
    <div class="grado-bloque-header">
        ...
        <label class="toggle-sw" title="{{ $habilitado ? 'Deshabilitar' : 'Habilitar' }} grado">
            <input type="checkbox"
                ...
                {{ $habilitado ? 'checked' : '' }}
                onchange="activarGrado({{ $ambiente->id }}, {{ $grado->id }}, '{{ addslashes($grado->nombre) }}', this)">
...
@push('scripts')
<script>
    const URL_ACTIVAR_GRADO = "{{ route('admin.ambientes.grados.activar', [$ambiente, ':grado']) }}";

    async function activarGrado(ambienteId, gradoId, nombre, input) {
        const url = URL_ACTIVAR_GRADO.replace(':grado', gradoId);
        try {
