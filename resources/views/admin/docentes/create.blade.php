...
@section('content')
<div class="page-header"><h1>Nuevo Docente</h1></div>
    ...
    <div class="form-group">
        ...
        <select name="rol" class="form-control" required>
            @foreach(['admin' => 'Administrador', 'docente_lider' => 'Docente Líder', 'docente_auxiliar' => 'Docente Auxiliar'] as $val => $label)
                <option value="{{ $val }}" {{ old('rol') === $val ? 'selected' : '' }}>{{ $label }}</option>
            @endforeach
        </select>
    </div>
    <div class="form-group">
        ...
        <select name="ambiente_id" class="form-control">
            ...
            @foreach($ambientes as $a)
                <option value="{{ $a->id }}" {{ old('ambiente_id') == $a->id ? 'selected' : '' }}>{{ $a->icono }} {{ $a->nombre }}</option>
            @endforeach
        </select>
    </div>
    @if($errors->any())
        <div style="background:#2D0F0F;border:1px solid #DC2626;border-radius:8px;padding:12px 16px;margin-bottom:16px;color:#FCA5A5;font-size:0.85rem">
            @foreach($errors->all() as $error)<div>{{ $error }}</div>@endforeach
        </div>
    @endif
    <div style="display:flex;gap:12px;margin-top:8px">
        <button type="submit" class="btn btn-primary">Crear Docente</button>
        <a href="{{ route('admin.docentes') }}" class="btn" style="background:#243447;color:#94A3B8">Cancelar</a>
    </div>
</form>
</div>
@endsection
