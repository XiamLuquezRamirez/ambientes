...
.wz-step    { display:flex; align-items:center; gap:10px; }
.wz-circle  {
    width:38px; height:38px; border-radius:50%;
    display:flex; align-items:center; justify-content:center;
    font-weight:700; font-size:.9rem; flex-shrink:0; transition:all .25s;
}
.wz-circle.pend { background:#E2E8F0; color:#94A3B8; }
.wz-circle.on   { background:#2563EB; color:#fff; box-shadow:0 0 0 4px #BFDBFE; }
.wz-circle.done { background:#22C55E; color:#fff; }
.wz-label       { font-size:.8rem; font-weight:600; color:#94A3B8; transition:color .2s; white-space:nowrap; }
.wz-label.on    { color:#1E293B; }
.wz-label.done  { color:#16A34A; }
.wz-line        { flex:1; height:2px; background:#E2E8F0; margin:0 10px; transition:background .3s; }
.wz-line.done   { background:#22C55E; }

/* ── Cards ─────────────────────────────────────────── */
.c-card {
    background:#fff; border:1px solid #E2E8F0;
    border-radius:16px; padding:28px; margin-bottom:0;
}
.c-head {
...
.est-row.marcado { background:#EFF6FF; }
.est-chk { width:16px; height:16px; accent-color:#2563EB; flex-shrink:0; }
.avatar  {
    width:36px; height:36px; border-radius:50%;
    display:flex; align-items:center; justify-content:center;
    color:#fff; font-weight:700; font-size:.8rem; flex-shrink:0;
}
.est-nom  { font-weight:600; color:#1E293B; font-size:.9rem; }
...
.est-mini:last-child { border-bottom:none; }

/* ── Dest summary ───────────────────────────────────── */
.dest-resumen {
    background:#F0FDF4; border:1px solid #BBF7D0;
    border-radius:10px; padding:14px 16px;
    font-size:.84rem; color:#166534; display:none; margin-top:14px;
}

/* ── Result ─────────────────────────────────────────── */
.result-wrap { text-align:center; padding:52px 24px; }
.result-ico  {
    width:76px; height:76px; border-radius:50%;
    background:#ECFDF5; color:#059669; font-size:2rem;
    margin:0 auto 20px; display:flex; align-items:center; justify-content:center;
}
.result-tit  { font-size:1.25rem; font-weight:700; color:#1E293B; margin-bottom:8px; }
.result-det  { font-size:.88rem; color:#64748B; margin-bottom:32px; line-height:1.6; }
...
@media (max-width:860px) { .p3-grid { grid-template-columns:1fr; } }

.campo-error { font-size:.76rem; color:#DC2626; margin-top:3px; min-height:16px; }
</style>
@endpush
...
async function cargarGrupos() {
    ...
    const { data } = await api(`${URL_GRUPOS_CIERRE}?anio=${_anio}`);
...
async function verEstudiantes() {
    ...
    if (!_grupoId) { errEl.textContent = 'Selecciona un grupo.'; return; }
    errEl.textContent = '';
    ...
    const { data } = await api(`${URL_EST_CIERRE}?anio=${_anio}&grupo_id=${_grupoId}`);

    btn.disabled = false;
    btn.innerHTML = 'Ver estudiantes <i class="fas fa-arrow-right ms-1"></i>';

    if (!data.ok) { mostrarToast('error', data.mensaje ?? 'Error.'); return; }
...
══════════════════════════════════════════════════ */
const ESTADO_BADGE = {
    promovido: { bg: '#EFF6FF', color: '#1D4ED8', txt: 'Promovido' },
    graduado:  { bg: '#FEF3C7', color: '#92400E', txt: 'Graduado'  },
    retirado:  { bg: '#FEF2F2', color: '#991B1B', txt: 'Retirado'  },
};
...
function renderP2() {
    ...
    document.getElementById('listaP2').innerHTML = _ests.map(e => `
        <label class="est-row ${_sel.has(e.matricula_id) ? 'marcado' : ''}"
            ...
            <span class="est-nom">${e.nombre}</span>
        </label>
    `).join('');
...
function actualizarContP2() {
    const n = _sel.size, tot = _ests.length;
...
async function renderP3() {
    ...
    document.getElementById('listaP3').innerHTML = selEsts.map(e => `
        <div class="est-mini">
            ...
            <span style="font-size:.87rem;font-weight:600;color:#1E293B">${e.nombre}</span>
        </div>
    `).join('');
    ...
    if (_estado === 'promovido') {
        ...
        const [bg, border, color] = _estado === 'graduado'
...
async function cargarGradosD() {
    ...
    const { data } = await api(`${URL_GRUPOS_DISP}?anio=${_anioDestino}`);
    if (!data.ok) {
...
function cargarGruposD() {
    ...
    if (!gradoId) { if (grupoW) grupoW.style.display = 'none'; return; }
...
function actualizarResumen() {
    ...
    if (!gradoId || !grupoId) { if (resumen) resumen.style.display = 'none'; return; }

    const grado = _grados.find(g => g.id === gradoId);
    const grupo = grado?.grupos.find(g => g.id === grupoId);
    if (!grado || !grupo) { if (resumen) resumen.style.display = 'none'; return; }
...
async function confirmarCierre() {
    const estadoTxt = { promovido:'promovidos', graduado:'graduados', retirado:'retirados' }[_estado];
    ...
    const payload = { matricula_ids: [..._sel], estado: _estado };
    if (_estado === 'promovido') {
        payload.grupo_destino_id = _grupoDestId;
        payload.anio_destino     = _anioDestino;
    }

    const { data } = await api(URL_APLICAR_CIERRE, 'POST', payload);
...
function nuevoCierre() {
    _ests = []; _sel.clear(); _grados = []; _grupoDestId = null;
    PASOS.forEach(id => {
        const el = document.getElementById(id);
        if (el) el.style.display = 'none';
    });
