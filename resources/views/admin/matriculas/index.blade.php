    ...
    <div class="modal-dialog modal-dialog-centered modal-lg">
        <div class="modal-content" style="border:none;border-radius:16px;overflow:hidden">
            ...
            <div class="modal-body" style="padding:24px">
                ...
                <div id="paso1">
                    <div style="display:grid;grid-template-columns:1.8fr 0.7fr 0.5fr;gap:10px;margin-bottom:12px">
                        <input type="text" id="inputFiltroEst" class="form-control"
                            placeholder="Filtrar por nombre…"
                            oninput="filtrarListaEstudiantes()">

                        <select id="filtroEdad" onchange="filtrarListaEstudiantes()" placeholder="Filtrar por edad…" style="width:100%" class="form-control" style="max-width:110px">
                            <option value="">Rango de edad…</option>
                            <option value="1-2">1-2 años</option>
                            <option value="2-3">3-4 años</option>
                            <option value="3-4">5-6 años</option>
                        </select>
...
<script>
    ...
    async function cargarTabla(url) {
        ...
        const {
            data
        } = await apiFetch(url || `${URL_TABLA}?${params}`);
    ...
    document.getElementById('filtroAnio').addEventListener('change', async () => {
        ...
        const {
            data
        } = await apiFetch(`${URL_GRUPOS_EST}?anio=${anio}`);
        if (data.ok) {
    ...
    document.getElementById('filtroBuscar').addEventListener('input', () => {
        clearTimeout(_deb);
        _deb = setTimeout(() => cargarTabla(), 400);
    });
    ...
    async function cargarEstudiantes() {
        ...
        _todosEstudiantes = _todosEstudiantes.map(e => ({
            ...e,
    ...
    function renderListaEstudiantes(lista) {
        ...
        <label class="est-fila ${_seleccionados.has(e.id) ? 'marcado' : ''}"
            ...
            <input type="checkbox" class="est-check" value="${e.id}"
                   ...
                   onclick="event.stopPropagation(); seleccionarEstudiante(${e.id}, this.closest('.est-fila'))">
            <div class="avatar-est" style="background:${e.color_avatar ?? '#2563EB'}">
                ${(e.iniciales ?? e.nombre.substring(0,2)).toUpperCase()}
            </div>
            <div>
                <div class="est-nombre">${e.nombre} (${e.edad} años)</div>
    ...
    function filtrarListaEstudiantes() {
        ...
        const termino = document.getElementById('inputFiltroEst').value.toLowerCase();
        let lista = _todosEstudiantes;
    ...
    function toggleTodosEstudiantes() {
        ...
        vis.forEach(e => {
            if (todos) _seleccionados.delete(e.id);
            else _seleccionados.add(e.id);
        });
    ...
    async function irPaso2() {
        ...
        const {
            data
        } = await apiFetch(`${URL_GRUPOS}?anio=${anio}`);
        if (!data.ok) {
    ...
    async function confirmarMatricula() {
        ...
        const {
            data
        } = await apiFetch(URL_MATRICULAS, 'POST', {
    ...
    /* ══════════════════════════════════════════════════════
       Editar grupo
       ══════════════════════════════════════════════════════ */
    let _modalEditarBS = null,
        _matriculaEditId = null;
    ...
    async function abrirEditarMatricula(id) {
        ...
        const {
            data
        } = await apiFetch(`/admin/matriculas/${id}/datos`);
        if (!data.ok) {
    ...
    /* ══════════════════════════════════════════════════════
       ...
       ══════════════════════════════════════════════════════ */
    let _modalEstadoBS = null,
        _matriculaEstId = null;
    ...
    async function aplicarEstadoGrupo() {
        ...
        const {
            data
        } = await apiFetch(URL_ESTADO_GRUPO, 'PATCH', {
            grupo_id: grupoId,
            estado
        });
    ...
    async function eliminarMatricula(id, nombre) {
        ...
        const {
            data
        } = await apiFetch(`/admin/matriculas/${id}`, 'DELETE');
        if (data.ok) {
