...
@section('content')
    <div class="page-header" style="display:flex;justify-content:space-between;align-items:center">
        ...
        <div style="display:flex;gap:10px">
            <button class="btn btn-primary" onclick="abrirModalCrearCatalogoDBA()">
                <i class="fas fa-plus"></i> Nuevo DBA personalizado
            </button>
        </div>
    </div>
    ...
    <form id="formBuscar" style="display:flex;gap:12px;margin-bottom:24px;align-items:center;flex-wrap:wrap">
        ...
        <a id="btnLimpiar" href="{{ route('admin.configuracion.catalogo-dba.listar') }}" class="btn btn-sm"
    ...
    <div id="contenedorTabla">
        @include('admin.configuracion.catalogos-DBA._tabla')
    </div>
...
@push('scripts')
    <script>
        const URL_CATALOGO_DBA_BASE = @json(url('admin/configuracion/catalogo-dba'));
        const URL_CATALOGO_DBA_GUARDAR = @json(route('admin.configuracion.catalogo-dba.guardar'));
        ...
        function setBtnCrearCatalogoDBA(modo) {
            ...
            if (modo === 'creando') {
                ...
                btn.innerHTML = '<i class="fa-solid fa-floppy-disk"></i> Crear DBA personalizado';
            } else {
        ...
        function abrirModalCrearCatalogoDBA() {
            tipoPost = 1;
            id_editar = '';
            $('#modalCrearCatalogoDBALabel').text('Crear DBA personalizado');
            $('#modalCrearCatalogoDBASubtitle').text('Define el DBA institucional de tu colegio');
        ...
        function abrirModalEditarCatalogoDBA(id) {
            tipoPost = 2;
            id_editar = id;
            $('#modalCrearCatalogoDBALabel').text('Editar DBA personalizado');
