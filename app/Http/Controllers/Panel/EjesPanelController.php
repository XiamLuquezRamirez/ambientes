...
class EjesPanelController extends Controller
{
    ...
    public function mover(Request $request, Eje $eje)
    {
        ...
        $vecino = Eje::query()
            ...
            ->where('creado_por', $eje->creado_por)
            ->when(
    ...
    private function serializarEje(Eje $eje, int $institucionId, ?bool $moduloActivo = null): array
    {
        ...
        $esMio = $eje->fueCreadoPor($this->docenteId());
    ...
    {
        $this->asegurarAmbienteAsignadoAlDocente($modulo->ambiente_id, $institucionId);
    ...
    private function asegurarEjePropio(Eje $eje): void
    {
        ...
        if (! $eje->fueCreadoPor($this->docenteId())) {
            abort(403, 'Solo puede gestionar ejes que usted creó.');
        }
    ...
    private function asegurarEjeGestionable(Eje $eje): void
    {
        ...
        if (! $this->moduloActivoParaInstitucion($eje->modulo, $this->institucionId())) {
            abort(422, 'El módulo no está activo. Solicite al administrador que lo active para gestionar ejes.');
        }
    }

    private function docenteId(): int
    {
        $id = Auth::guard('docente')->user()?->docente?->id;
