...
class CondicionTransitoriaController extends Controller
{
    public function index(Request $request)
    {
        ...
        $consulta = CondicionTransitoria::query()
        ...
        $condicionesBase = CondicionInclusion::ordenadas()->get(['id', 'codigo', 'nombre', 'color_hex', 'estado']);

        if ($request->ajax()) {
            return response()->json([
                'success' => true,
                'html' => view('superAdmin.condicionesTransitorias._tabla', [
        ...
        return view('superAdmin.condicionesTransitorias.index', [
            'condiciones' => $condiciones,
    ...
    public function guardar(Request $request)
    {
        ...
        $datos['codigo'] = CondicionTransitoria::generarCodigo();
        $datos['estado'] = 1;
        ...
        $condicion = CondicionTransitoria::create($datos);
    ...
    public function cambiarEstado(CondicionTransitoria $condicionTransitoria)
    {
        $this->autorizarAcceso($condicionTransitoria);
    ...
    private function autorizarAcceso(CondicionTransitoria $condicion): void
    {
        $usuario = Auth::guard('docente')->user();
