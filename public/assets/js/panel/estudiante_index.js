...
function contenedorPinDocente() {
    return $("#configuracion_pin_docente");
}

function activarContenedorPinDocente() {
    idContainerPin = "configuracion_pin_docente";
}

function restaurarUiPinDocenteEditable() {
    const $scope = contenedorPinDocente();
    $scope.find(".figuras-container").show();
    $scope.find(".titulo-configuracion-pin").text(
        "Seleccione 3 figuras para conformar el PIN de acceso del estudiante"
    );
    $("#modalConfigurarPinLabel").text("Configurar PIN");
    $("#modalConfigurarPin .modal-footer").show();
}

function abrirModalConfigurarPin(id, nombre) {
    activarContenedorPinDocente();
    reiniciarPin();
    restaurarUiPinDocenteEditable();
...
function abrirModalVerPinEstudiante(figura1, figura2, figura3, colorfigura1, colorfigura2, colorfigura3, nombre) {
    ...
    var datos = {
        ...
        figura_2: figura2,
        figura_3: figura3,
        color_figura_1: colorfigura1,
        color_figura_2: colorfigura2,
        color_figura_3: colorfigura3
    };
    mapearDatosPin(datos);

    const $scope = contenedorPinDocente();
    $scope.find(".figuras-container").hide();
    $scope.find(".titulo-configuracion-pin").text("PIN de acceso del estudiante");
    $("#modalConfigurarPinLabel").text("PIN configurado");
