...
(function ($) {
            ...
            <div class="rn-camino-root">
                ...
                <div class="rn-camino-viewport" id="rnCaminoViewport">
                    <div class="rn-camino-mundo" id="rnCaminoMundo">
                        <svg class="rn-camino-svg" viewBox="0 0 100 100" preserveAspectRatio="none" aria-hidden="true">
                            ...
                            <path class="rn-camino-ruta-progreso" id="rnCaminoProgreso" d=""></path>
                        </svg>
    ...
    function actualizarProgresoRuta() {
        const prog = document.getElementById('rnCaminoProgreso');
        if (!prog) return;
        // El progreso dibuja SOLO el tramo recorrido: sub-path desde el
        // punto 0 hasta el último punto visitado. Nada recorrido => vacío
        // (carretera toda gris). Robusto: no depende de dash/offset ni de
        // getTotalLength (que falla con vector-effect: non-scaling-stroke).
        if (indiceMaximoVisitado <= 0) {
            prog.setAttribute('d', '');
            return;
        }
        const puntosRecorridos = camino.puntos.slice(0, indiceMaximoVisitado + 1);
        prog.setAttribute('d', pathSuave(puntosRecorridos));
    }
    ...
    function enlazarEventos() {
        ...
        ctx.$paso.on('click.rnCamino', '#rnCaminoIniciar', function (e) {
            e.preventDefault();
            iniciarRecorrido();
        });
    ...
    function render() {
        ...
        $(window).off('resize.rnCamino').on('resize.rnCamino', function () {
            ...
            reposicionarBotonIniciar();
        });
    }

    /* Estado inicial guiado: el personaje espera y aparece el botón "¡Iniciar!".
       Al tocarlo, camina a la primera parada real. */
    function mostrarBotonIniciar() {
        $('.rn-camino-iniciar').remove();
        if (indiceActual !== 0 || camino.paradas.length < 2) return;
        ctx.$shell.addClass('rn-camino-shell--espera');
        const $btn = $(
            '<button type="button" class="rn-camino-iniciar" id="rnCaminoIniciar" aria-label="Iniciar el recorrido">'
            + '<span class="rn-camino-iniciar__brillo" aria-hidden="true"></span>'
            + '<span class="rn-camino-iniciar__texto">¡Iniciar!</span>'
            + '<span class="rn-camino-iniciar__flecha" aria-hidden="true">▶</span>'
            + '</button>'
        );
        ctx.$paso.find('.rn-camino-viewport').append($btn);
        reposicionarBotonIniciar();
    }

    function reposicionarBotonIniciar() {
        const $btn = $('.rn-camino-iniciar');
        if (!$btn.length) return;
        const pt = camino.puntos[0] || { x: 8, y: 52 };
        // Mantener el botón dentro del viewport aunque el punto de inicio
        // esté muy al borde (evita que se recorte a la izquierda).
        const left = Math.min(Math.max(pt.x, 16), 84);
        $btn.css({ left: left + '%', top: (pt.y + 16) + '%' });
    }

    function ocultarBotonIniciar() {
        ctx.$shell.removeClass('rn-camino-shell--espera');
        $('.rn-camino-iniciar').addClass('is-saliendo');
        setTimeout(function () { $('.rn-camino-iniciar').remove(); }, 260);
    }

    function iniciarRecorrido() {
        if (caminando || indiceActual !== 0) return;
        ocultarBotonIniciar();
        caminarA(1);
    }
