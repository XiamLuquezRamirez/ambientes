/* Busca la sombra — Multisensorial */
(function () {
    "use strict";
    ...
    function metaObj(id) {
        return (gameConfig.objetos && gameConfig.objetos[id]) || {
            nombre: id,
            etiqueta: id,
            img: "",
            sombra: ""
    ...
    function urlsImagenesCriticas() {
        ...
        Object.keys(gameConfig.objetos || {}).forEach(function (id) {
            ...
            if (m.sombra) urls.push(m.sombra);
        });
        return urls;
    }
    ...
    function frasesFijasTts() {
        ...
        const frases = [
            ...
            "¡Inténtalo otra vez! Observa muy bien su forma.",
            "¡Excelente! Encontraste todas las sombras."
        ];
        Object.keys(gameConfig.objetos || {}).forEach(function (id) {
            const m = metaObj(id);
            frases.push(plantilla(t.consignaPlantilla || "Encuentra la sombra de {objeto}.", { objeto: m.nombre }));
            frases.push(plantilla(t.aciertoPlantilla || "¡Muy bien! Encontraste la sombra de {objeto}.", { objeto: m.nombre }));
            frases.push(plantilla(t.aciertoPlantillaAvanzada || "¡Excelente! Encontraste la sombra de {objeto}.", { objeto: m.nombre }));
        });
    ...
    async function iniciarDemo(gen) {
        ...
        if (!retoActual || !retoActual.objeto) {
            detenerDemo();
            iniciarConsigna(gen);
            return;
        }
        ...
        pintarObjetoObjetivo(retoActual.objeto);
        pintarSombras(shuffle((retoActual.sombras || []).slice()));

        const msg = textos().demostracion ||
            "¡Vamos con un ejemplo! Mira cómo se hace: observa el objeto y toca su sombra.";
        const enunciado = document.getElementById("enunciado");
        if (enunciado) enunciado.textContent = msg;
        await hablarDemo(msg, 3000);

        const targetId = retoActual.objeto;
        const targetBtn = Array.prototype.find.call(document.querySelectorAll(".sombra"), function (btn) {
            return btn.dataset.sombra === targetId;
        });
        ...
        const metaAcierto = metaObj(retoActual.objeto);
        const textoAcierto = plantilla(
            textos().aciertoPlantilla || "¡Muy bien! Encontraste la sombra de {objeto}.",
            { objeto: metaAcierto.nombre }
        );
        ...
        detenerDemo();
        const fin = textos().demostracionFin || "¡Ahora te toca a ti! Encuentra la sombra correcta.";
    ...
    function pintarObjetoObjetivo(objetoId) {
        const cont = document.getElementById("objeto-objetivo");
        if (!cont) return;
        const m = metaObj(objetoId);
        cont.innerHTML =
            '<img src="' + escAttr(m.img) + '" alt="' + escAttr(m.etiqueta || m.nombre || objetoId) + '">' +
            '<span class="objeto-objetivo-nombre">' + escHtml(m.etiqueta || m.nombre || objetoId) + "</span>";
    }
    ...
    function iniciarConsigna(gen) {
        ...
        const meta = metaObj(retoActual.objeto);
        const consigna = plantilla(
            textos().consignaPlantilla || "Encuentra la sombra de {objeto}.",
            { objeto: meta.nombre }
        );
        const enunciado = document.getElementById("enunciado");
    ...
    function pintarSombras(ids) {
        const zona = document.getElementById("zona-sombras");
        if (!zona) return;
        zona.innerHTML = "";
        ...
        ids.forEach(function (id) {
            ...
            btn.type = "button";
            btn.className = "sombra";
            btn.dataset.sombra = id;
            btn.setAttribute("aria-label", "Sombra de " + (m.etiqueta || m.nombre || id));
            btn.innerHTML = '<img src="' + escAttr(m.sombra || m.img) + '" alt="">';
    ...
    async function resolverToque(sombraId, el) {
        if (demoActiva || !aceptaToque || esperandoFeedback || juegoTerminado || !retoActual) return;
        const correcto = sombraId === retoActual.objeto;
        if (!correcto) {
    ...
    async function feedbackAcierto() {
        ...
        aceptaToque = false;
        const meta = metaObj(retoActual.objeto);
        const tpl = nivelEsAvanzado()
            ? (textos().aciertoPlantillaAvanzada || textos().aciertoPlantilla)
            : textos().aciertoPlantilla;
        const texto = plantilla(
            tpl || "¡Muy bien! Encontraste la sombra de {objeto}.",
            { objeto: meta.nombre }
        );
    ...
    async function feedbackError() {
        ...
        const texto = nivelEsAvanzado()
            ? (textos().errorAvanzado || textos().error || "¡Inténtalo otra vez! Observa con atención su forma y sus detalles.")
            : (textos().error || "¡Inténtalo otra vez! Observa muy bien su forma.");
