/* Completa La Figura — Multisensorial */
(function () {
    "use strict";
    ...
    function metaFigura(id) {
        return (gameConfig.figuras && gameConfig.figuras[id]) || {
            nombre: id,
            etiqueta: id,
            base: "",
            completa: "",
    ...
    function metaPieza(id) {
        return (gameConfig.piezas && gameConfig.piezas[id]) || { nombre: id, img: "" };
    }
    ...
    function urlsImagenesCriticas() {
        const urls = [];
        Object.keys(gameConfig.figuras || {}).forEach(function (id) {
            const m = metaFigura(id);
            if (m.base) urls.push(m.base);
            if (m.completa) urls.push(m.completa);
        });
        Object.keys(gameConfig.piezas || {}).forEach(function (id) {
            const m = metaPieza(id);
    ...
    function frasesFijasTts() {
        ...
        const frases = [
            ...
            "¡Inténtalo otra vez! Observa qué parte le falta.",
            "¡Excelente! Completaste todas las figuras."
        ];
        Object.keys(gameConfig.figuras || {}).forEach(function (id) {
            const m = metaFigura(id);
            frases.push(plantilla(t.consignaPlantilla || "Completa {figura}.", { figura: m.nombre }));
            frases.push(plantilla(t.aciertoPlantilla || "¡Muy bien! Completaste {figura}.", { figura: m.nombre }));
        });
    ...
    window.confirmarNivel = function confirmarNivel(id) {
        ...
        aceptaToque = false;
        TextoVoz.detener();
        retosRonda = (nivelElegido.retos || []).slice();
        indiceReto = 0;
    ...
    async function iniciarDemo(gen) {
        ...
        retoActual = retosRonda[0] || null;
        if (!retoActual || !retoActual.figura) {
            detenerDemo();
            iniciarConsigna(gen);
            return;
        }
        figuraActual = metaFigura(retoActual.figura);

        const prog = document.getElementById("progreso");
        if (prog) prog.hidden = true;

        pintarFigura(retoActual.figura, false);
        pintarPiezas(shuffle((retoActual.opciones || []).slice()));

        const msg = textos().demostracion ||
            "¡Vamos con un ejemplo! Mira cómo se hace: observa la figura y toca la pieza que le falta.";
        const enunciado = document.getElementById("enunciado");
        if (enunciado) enunciado.textContent = msg;
        await hablarDemo(msg, 3000);

        const targetId = figuraActual.piezaCorrecta || retoActual.figura;
        const targetBtn = Array.prototype.find.call(document.querySelectorAll(".pieza"), function (btn) {
            return btn.dataset.pieza === targetId;
        });
        ...
        detenerDemo();
        const fin = textos().demostracionFin || "¡Ahora te toca a ti! Completa cada figura.";
    ...
    function actualizarProgreso() {
        ...
        el.hidden = !total;
        el.innerHTML = '<i class="fa-solid fa-puzzle-piece"></i> ' +
            Math.min(indiceReto + 1, total) + " / " + total;
    }

    function pintarFigura(figuraId, completa) {
        const img = document.getElementById("figura-img");
        const nombre = document.getElementById("figura-nombre");
        const escena = document.getElementById("figura-escena");
        const m = metaFigura(figuraId);
        if (img) {
            img.src = completa ? (m.completa || m.base) : m.base;
            img.alt = m.etiqueta || m.nombre || figuraId;
        }
        if (nombre) nombre.textContent = m.etiqueta || m.nombre || figuraId;
        if (escena) {
            escena.classList.toggle("is-completa", !!completa);
        }
    }

    function iniciarConsigna(gen) {
        if (gen !== rondaGen || juegoTerminado) return;
        retoActual = retosRonda[indiceReto] || null;
        if (!retoActual || !retoActual.figura) {
            mostrarCierre();
            return;
        }
        ...
        const consigna = plantilla(
            textos().consignaPlantilla || "Completa {figura}.",
            { figura: figuraActual.nombre }
        );
        const enunciado = document.getElementById("enunciado");
    ...
    function pintarPiezas(ids) {
        const zona = document.getElementById("zona-piezas");
        if (!zona) return;
        zona.innerHTML = "";
        zona.dataset.count = String(ids.length);
        ids.forEach(function (id) {
            const m = metaPieza(id);
            const btn = document.createElement("button");
            btn.type = "button";
            btn.className = "pieza";
            btn.dataset.pieza = id;
            btn.setAttribute("aria-label", m.nombre || id);
            btn.innerHTML =
                '<img src="' + escAttr(m.img) + '" alt="">' +
                '<span class="pieza-nombre">' + escHtml(m.nombre || id) + "</span>";
    ...
    async function resolverToque(piezaId, el) {
        if (demoActiva || !aceptaToque || esperandoFeedback || juegoTerminado || !retoActual || !figuraActual) return;
        const correcta = figuraActual.piezaCorrecta || retoActual.figura;
        const ok = piezaId === correcta;
        if (!ok) {
            el.classList.add("is-error");
            setTimeout(function () { el.classList.remove("is-error"); }, 420);
            await feedbackError();
            return;
        }
        ...
        if (indiceReto + 1 < retosRonda.length) {
            indiceReto += 1;
            await sleep(280);
            iniciarConsigna(rondaGen);
            return;
    ...
    async function feedbackError() {
        ...
        const texto = nivelEsAvanzado()
            ? (textos().errorAvanzado || textos().error || "¡Inténtalo otra vez! Observa muy bien la forma y el tamaño de la pieza.")
            : (textos().error || "¡Inténtalo otra vez! Observa qué parte le falta.");
