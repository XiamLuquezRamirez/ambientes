...
/** Evita que .cls-* / id de un SVG pisen a otros en el mismo documento. */
function aislarEstilosSvg(raiz) {
    if (!raiz || raiz.nodeType !== 1) return;
    const prefijo = "es" + (++svgUidSeq) + "-";
    const mapaClases = Object.create(null);
    const mapaIds = Object.create(null);

    function claseUnica(nombre) {
        if (!mapaClases[nombre]) mapaClases[nombre] = prefijo + nombre;
        return mapaClases[nombre];
    }

    function idUnico(nombre) {
        if (!mapaIds[nombre]) mapaIds[nombre] = prefijo + nombre;
        return mapaIds[nombre];
    }

    function reescribirRefs(valor) {
        if (!valor || valor.indexOf("#") < 0) return valor;
        return valor.replace(/url\(\s*#([^)]+?)\s*\)/gi, function (_, id) {
            return "url(#" + idUnico(String(id).trim()) + ")";
        });
    }

    raiz.querySelectorAll("style").forEach(function (styleEl) {
        const texto = styleEl.textContent || "";
        styleEl.textContent = texto.replace(/\.([A-Za-z_][\w-]*)/g, function (_, cls) {
            return "." + claseUnica(cls);
        });
    });

    const todos = [raiz].concat(Array.prototype.slice.call(raiz.querySelectorAll("*")));
    todos.forEach(function (el) {
        ...
        if (id) el.setAttribute("id", idUnico(id));

        ["fill", "stroke", "filter", "clip-path", "mask", "style"].forEach(function (attr) {
            const v = el.getAttribute(attr);
            if (!v) return;
            const nuevo = reescribirRefs(v);
            if (nuevo !== v) el.setAttribute(attr, nuevo);
        });
        const href = el.getAttribute("href") || el.getAttribute("xlink:href");
        if (href && href.charAt(0) === "#") {
            const nid = "#" + idUnico(href.slice(1));
            if (el.hasAttribute("href")) el.setAttribute("href", nid);
            if (el.hasAttribute("xlink:href")) el.setAttributeNS("http://www.w3.org/1999/xlink", "href", nid);
        }
    });
}

function importarPiezaSvg(archivo, variante) {
    const NS = "http://www.w3.org/2000/svg";
    const wrap = document.createElementNS(NS, "g");
    extraerHijosSvg(readText(rutaPieza(archivo, variante))).forEach(function (nodo) {
        wrap.appendChild(document.importNode(nodo, true));
    });
    aislarEstilosSvg(wrap);
    return wrap;
}
...
function appendCapaSvg(destino, archivo, variante, clase) {
    ...
    const wrap = importarPiezaSvg(archivo, variante);
    while (wrap.firstChild) capa.appendChild(wrap.firstChild);
    destino.appendChild(capa);
    return capa;
}

/** Los SVG gris/color del robot no comparten el mismo origen; centra color sobre gris. */
function alinearCapaColorAGris(parte) {
    const gris = parte.querySelector(".svg-parte-gris");
    const color = parte.querySelector(".svg-parte-color");
    if (!gris || !color) return;
    try {
        const bg = gris.getBBox();
        const bc = color.getBBox();
        if (!bg.width || !bg.height || !bc.width || !bc.height) return;
        const dx = bg.x + bg.width / 2 - (bc.x + bc.width / 2);
        const dy = bg.y + bg.height / 2 - (bc.y + bc.height / 2);
        if (Math.abs(dx) < 0.05 && Math.abs(dy) < 0.05) {
            color.removeAttribute("transform");
            return;
        }
        color.setAttribute("transform", "translate(" + dx + " " + dy + ")");
    } catch (e) {
        /* getBBox puede fallar si aún no está en el DOM */
    }
}
...
function armarSiluetaSvg(layout) {
    ...
    svg.querySelectorAll(".svg-parte").forEach(alinearCapaColorAGris);
...
function iniciarPartida() {
    // Género aleatorio (nina/nino); la edad define el número de piezas
    const opciones = [];
    if (gameConfig.cuerpos) {
        if (gameConfig.cuerpos.nina) opciones.push("nina");
        if (gameConfig.cuerpos.nino) opciones.push("nino");
        if (!opciones.length) opciones.push.apply(opciones, Object.keys(gameConfig.cuerpos));
    }
    cuerpoElegido = opciones.length
...
function pintarFichaSvg(ficha, pieza) {
    ...
    svg.setAttribute("preserveAspectRatio", "xMidYMid meet");
    const wrap = importarPiezaSvg(archivo, "color");
